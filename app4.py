...
for index, problem in enumerate(problems):
    ...
    st.components.v1.html(f"<div>{transformed_question}</div>", height=300)  # ✅ 문제를 MathJax로 변환된 상태로 출력
    ...
    st.components.v1.html(f"<div>{transformed_explanation}</div>", height=300)  # ✅ 변환된 해설을 MathJax로 변환된 상태로 출력
